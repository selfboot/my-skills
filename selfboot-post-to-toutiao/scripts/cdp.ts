...
export function findChromeExecutable(): string | undefined {
  const override = process.env.TOUTIAO_BROWSER_CHROME_PATH?.trim();
...
export function getDefaultProfileDir(): string {
  const override = process.env.TOUTIAO_BROWSER_PROFILE_DIR?.trim();
  if (override) return path.resolve(override);
  const base = process.platform === 'darwin'
    ? path.join(os.homedir(), 'Library', 'Application Support')
    : process.env.XDG_DATA_HOME || path.join(os.homedir(), '.local', 'share');
  return path.join(base, 'selfboot-skills', 'toutiao-chrome-profile');
}
...
export async function launchChrome(url: string, profileDir?: string): Promise<{ cdp: CdpConnection; chrome: ReturnType<typeof spawn> }> {
  ...
  if (!chromePath) throw new Error('Chrome not found. Set TOUTIAO_BROWSER_CHROME_PATH env var.');

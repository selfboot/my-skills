...
export async function convertMarkdown(
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "toutiao-article-images-"));
  const htmlPath = path.join(tempDir, "temp-article.html");

  console.error(
    `[md-to-toutiao] Rendering markdown with theme: ${options?.theme ?? "default"}${options?.color ? `, color: ${options.color}` : ""}, citeStatus: ${citeStatus}`,
  );
  ...
  const contentImages = await resolveContentImages(images, baseDir, tempDir, "md-to-toutiao");
...
function printUsage(): never {
  console.log(`Convert Markdown to Toutiao-ready HTML with image placeholders

Usage:
  npx -y bun md-to-toutiao.ts <markdown_file> [options]
...
{
  "title": "Article Title",
  "htmlPath": "/tmp/toutiao-article-images/temp-article.html",
  "contentImages": [
    {
      "placeholder": "TOUTIAOIMGPH_1",
      "localPath": "/tmp/toutiao-image/img.png",
...
Example:
  npx -y bun md-to-toutiao.ts article.md
  npx -y bun md-to-toutiao.ts article.md --theme grace
  npx -y bun md-to-toutiao.ts article.md --theme modern --color blue
  npx -y bun md-to-toutiao.ts article.md --no-cite
`);
  process.exit(0);
}

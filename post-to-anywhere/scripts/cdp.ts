...
export function findChromeExecutable(): string | undefined {
  const override = process.env.POST_TO_ANYWHERE_BROWSER_CHROME_PATH?.trim()
    || process.env.TOUTIAO_BROWSER_CHROME_PATH?.trim();
...
export function getDefaultProfileDir(profileName = 'default'): string {
  const override = process.env.POST_TO_ANYWHERE_BROWSER_PROFILE_DIR?.trim()
    || process.env.TOUTIAO_BROWSER_PROFILE_DIR?.trim();
  if (override) return path.resolve(override);
  const base = process.platform === 'darwin'
    ? path.join(os.homedir(), 'Library', 'Application Support')
    : process.env.XDG_DATA_HOME || path.join(os.homedir(), '.local', 'share');
  const profileSuffix = profileName === 'default' ? 'chrome-profile' : `${profileName}-chrome-profile`;
  return path.join(base, 'selfboot-skills', 'post-to-anywhere', profileSuffix);
}
...
export async function launchChrome(
  ...
  if (!chromePath) throw new Error('Chrome not found. Set POST_TO_ANYWHERE_BROWSER_CHROME_PATH env var.');

  const profile = profileDir ?? getDefaultProfileDir(profileName);

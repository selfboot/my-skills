...
export async function convertMarkdown(
  markdownPath: string,
  options?: { title?: string; theme?: string; color?: string; citeStatus?: boolean; platform?: string },
): Promise<ParsedResult> {
  const baseDir = path.dirname(markdownPath);
  const content = fs.readFileSync(markdownPath, "utf-8");
  const citeStatus = options?.citeStatus ?? true;
  const platform = (options?.platform ?? "article").toUpperCase().replace(/[^A-Z0-9]/g, "");
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "post-to-anywhere-article-"));
  const htmlPath = path.join(tempDir, "temp-article.html");

  console.error(
    `[md-to-article] Rendering markdown for ${options?.platform ?? "generic"} with theme: ${options?.theme ?? "default"}${options?.color ? `, color: ${options.color}` : ""}, citeStatus: ${citeStatus}`,
  );
  ...
  const contentImages = await resolveContentImages(images, baseDir, tempDir, "md-to-article");
...
function printUsage(): never {
  console.log(`Convert Markdown to paste-ready HTML with image placeholders

Usage:
  npx -y bun md-to-article.ts <markdown_file> [options]

Options:
  --platform <id>     Platform id used for placeholder prefix
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i]!;
    if (arg === "--platform" && args[i + 1]) platform = args[++i];
  ...
  const result = await convertMarkdown(markdownPath, { platform, title, theme, color, citeStatus });
